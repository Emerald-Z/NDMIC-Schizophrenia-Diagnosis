...
class LSTMNet(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.lstm1 = nn.LSTM(input_size=441, hidden_size=36, batch_first=True, num_layers=1)
        ...
        # 441 time steps
        self.fc1 = nn.Linear(128 * 36, num_classes)
        # self.fc1 = nn.Linear(72, num_classes)
...
class CNNLSTMModel(nn.Module):
    ...
    def forward(self, x):
        x = self.Conv3DNet(x)
        x = x.view(x.size(0), -1, 441)  # Adjust shape for LSTM
        x = self.LSTMNet(x)
        return x
...
def train_model(model, criterion, optimizer, train_loader, val_loader,
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="min", factor=0.7) # reduce lr?
    ...
    for epoch in range(num_epochs):
        ...
        scheduler.step(epoch_loss) # reduce lr?

        print(f'Epoch {epoch+1}/{num_epochs} - Training Loss: {epoch_loss:.4f}, Acc: {epoch_acc:.4f}')

        # Call eval loop for validation
        eval_model(model, criterion, val_loader, epoch, True) # put in epoch for 

# Evaluate the model -----------------------------------------------------------
def eval_model(model, criterion, val_loade, epoch, epoch_vis=True):
    ...
    if epoch_vis:
        writer.add_scalar("Eval Loss / Train", epoch_loss, epoch) # For visualization
        writer.add_scalar("Eval Acc / Train", epoch_acc, epoch) # For visualization
...
train_dataset = NeuroimagingDataset(
    '/home/neuro/Desktop/Schizophrenia_Project/Data/COBRE/rearranged_data/ResultsS',
    '/home/neuro/Desktop/Schizophrenia_Project/Data/COBRE/phenotypic_data.csv',
    feature_types='ALFF')
...
# Maybe we could try run-wise CV?
...
for fold, (train_idx, test_idx) in enumerate(kf.split(train_dataset)):
    ...
    # Load weights from 50 epochs full run
    # model.load_state_dict(torch.load('/Users/weavejul/Documents/Class_and_Lab/My_Lab_Spring_24/Data/COBRE/model_checkpoint_e'))
    ...
    eval_model(model, criterion, test_loader, None, False) # No vis
