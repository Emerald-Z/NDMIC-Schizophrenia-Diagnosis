...
class LitClassifier(pl.LightningModule):
    ...
    def relprop(self, cam, **kwargs):
        return self.output_head.relprop(cam, **kwargs)
    ...
    def _compute_logits(self, batch, augment_during_training=None):
        ...
        if self.hparams.downstream_task == 'schizo' or self.hparams.downstream_task_type == 'classification' or self.hparams.scalability_check:
            ...
            real_values = torch.stack(schizo, dim=0).squeeze()
            # Structured like this: [not schizo prob, schizo prob]
            target = torch.tensor([[1-value, value] for value in real_values]).to(real_values.device)
        # Regression task
        elif self.hparams.downstream_task == 'age' or self.hparams.downstream_task == 'int_total' or self.hparams.downstream_task == 'int_fluid' or self.hparams.downstream_task_type == 'regression':
    ...
    def _calculate_loss(self, batch, mode):
        if self.hparams.pretraining:
            ...
            if self.hparams.downstream_task == 'schizo' or self.hparams.downstream_task_type == 'classification' or self.hparams.scalability_check:
                ...
                loss = F.cross_entropy(logits, target.float())
                # acc = self.metric.get_accuracy_binary(logits, target.float().squeeze())
                acc = self.metric.get_accuracy(logits, target.argmax(dim=1).float())
                result_dict = {
                    f"{mode}_loss": loss,
                    f"{mode}_acc": acc,
                }
    ...
    def _evaluate_metrics(self, subj_array, total_out, mode):
        ...
        for subj in subjects:
            target = torch.nonzero(total_out[subj_array == subj,1][0, :])
            subj_logits = total_out[subj_array == subj,0][target[0].item()] 
    ...
    def add_model_specific_args(parent_parser):
        ...
        group.add_argument("--patch_size", nargs="4", default=[6, 6, 6, 1], type=int, help="patch size")
