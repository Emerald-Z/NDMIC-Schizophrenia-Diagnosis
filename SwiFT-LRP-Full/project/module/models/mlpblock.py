...
class MLPBlock(nn.Module):
    ...
    def __init__(self, hidden_size: int, mlp_dim: int, dropout_rate: float = 0.0, act: tuple | str = "GELU", dropout_mode="vit", labels={}) -> None:
        """
        Args:
        ...
        self.linear1 = Linear(in_features=hidden_size, out_features=mlp_dim, labels=labels["l1"]) if act != "GEGLU" else Linear(in_features=hidden_size, out_features=mlp_dim * 2, labels=labels["l1"])
        self.linear2 = Linear(in_features=mlp_dim, out_features=hidden_size, labels=labels["l2"])
        # TODO currently no definition of get_act_layer in SwifT-LRP
        self.fn = GELU(labels=labels["gelu"]) #get_act_layer(act) #  change this to GELU if we're sure
        self.drop1 = Dropout(p=dropout_rate, labels=labels["do"])
        dropout_opt = look_up_option(dropout_mode, SUPPORTED_DROPOUT_MODE)
        if dropout_opt == "vit":
            self.drop2 = Dropout(p=dropout_rate, labels=labels["do"])
