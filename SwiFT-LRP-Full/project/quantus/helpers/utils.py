...
def infer_channel_first(x: np.array) -> bool:
    ...
    # allow 3D input
    return True # TODO: fix this when we release code or not LOL
...
def expand_attribution_channel(a_batch: np.ndarray, x_batch: np.ndarray):
    ...
    """
    print("batch shapes: ", a_batch.shape, x_batch.shape)
    if len(a_batch.shape) != 6:
        a_batch = np.expand_dims(a_batch, 1)
    # a_batch = a_batch.swapaxes(0, 1)
