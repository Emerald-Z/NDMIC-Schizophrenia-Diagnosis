...
if util.find_spec("tf_explain"): # TODO: findspec is found by importlib
    import tf_explain
...
def get_explanation(model, inputs, targets, **kwargs):
    ...
    if method == "TransLRP":
        return generate_translrp_explanation(model, inputs, targets, **kwargs)
...
def generate_translrp_explanation(
    model,
    inputs: np.ndarray,
    targets: np.ndarray,
    ...
    **kwargs,
    ...
    if method in constants.DEPRECATED_XAI_METHODS_CAPTUM:
    ...
    elif method == "TransLRP":
        alpha = kwargs.get("alpha")
        e = kwargs.get("epsilon")
        g = kwargs.get("gamma")
        lrp_instance = LRP(model)
        if not torch.is_tensor(inputs):
            inputs = torch.from_numpy(inputs).float().to("cuda")
        explanation = lrp_instance.generate_LRP(inputs, alpha=alpha, epsilon=e, gamma=g)
        ...
        raise KeyError(
            f"The selected {method} XAI method is not in the list of supported built-in Quantus XAI methods for Translrp. "
    ...
    **kwargs,
    ...
    if method in ["GradientShap", "DeepLift", "DeepLiftShap"]:
        ...
        if "feature_mask" in kwargs:
            extra["feature_mask"] = kwargs["feature_mask"]
        if "interpretable_model" in kwargs:
            xai_lib_kwargs["interpretable_model"] = kwargs["interpretable_model"]
        explanation = f_reduce_axes(
            attr_func(model, **xai_lib_kwargs).attribute(inputs=inputs, target=targets, **extra)
        )
